 ...
 * @param {number} [minSentences=1] Minimum amount of sentences
 * @param {number} [maxSentences=minSentences] Maximum amount of sentences
 */
export default function lipsum(minSentences = 1, maxSentences = minSentences) {
    /**
     * @param {number} minWords Minimum amount of words per sentence
     * @param {number} maxWords Maximum amount of words per sentence
     * @return {Array<string[]>}
     */
    return function(minWords = 5, maxWords = minWords) {
        /** @tpye Array<string[]> */
        const lorem = [];
        const numberOfSentences = randomInt(minSentences, maxSentences);

        for (let i = 0; i < numberOfSentences; i += 1) {
            const count = randomInt(minWords, maxWords);
            const wordIndex = randomInt(0, words.length - count - 1);
...
/**
 * Convert a function that return string[][] to return string
 * @param {Function} fn A function that return function|string[][]
 */
export function toSentence(fn) {
    return function(...args) {
        /** @type Function | Array<string[]> */
        const ret = fn(...args);

        if (typeof ret === "function") {
            return toSentence(ret);
        }
